...
START_TEST(dec_to_flt_1) {
  ...
  int my_error = s21_from_decimal_to_float(number, NULL);
  int result_error = 1;
  ck_assert_int_eq(result_error, my_error);
}
END_TEST
...
Suite *suite_from_decimal_to_float() {
  Suite *s = suite_create(YELLOW_BG BLACK "s21_from_decimal_to_float" RESET);
  TCase *tc = tcase_create("decimal_from_decimal_to_float_tc");
  ...
  tcase_add_test(tc, dec_to_flt_1);
